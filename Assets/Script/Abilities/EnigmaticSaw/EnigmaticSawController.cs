    ...
    private GameObject GetPooledObject() {
        for (int i = 0; i < ObjectsList.Count; i++) {
            if (!ObjectsList[i].activeInHierarchy) {
                ...
                ObjectsScriptList[i].UpdateStat(new int[] {AbilitiesStat.Damage}, new float[] { AbilitiesStat.Knockback, Duration, DamageInterval, AdditionalScale });
        ...
        ObjectNew.GetComponent<ProjectileController>().UpdateStat(new int[] {AbilitiesStat.Damage}, new float[] {AbilitiesStat.Knockback, Duration, DamageInterval, AdditionalScale});
