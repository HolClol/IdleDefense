...
public class EruptionController : ProjectileController
{
    ...
    private void OnTriggerStay2D(Collider2D trigger) {
        if (trigger.gameObject.CompareTag("Enemy")) {
            if (BurningGround) {
                ResponseDamage.Invoke(trigger.gameObject, new int[] { Damage / 2, ID }, new float[] { Knockback / 10, 1f });
            }
            else {
                ResponseDamage.Invoke(trigger.gameObject, new int[] { Damage, ID }, new float[] { Knockback, 1f });
            }
